...
describe('UMD bundle', () => {
  beforeAll(() => {
    ...
    if (!fs.existsSync(umdPath)) {
      throw new Error('dist/main.umd.js not found. Run `npm run build` before running jest')
    }
    const code = fs.readFileSync(umdPath, 'utf8')

    // Provide explicit runtime fallbacks that some UMD shapes expect
    try { globalThis.Encoding = require('encoding-japanese') } catch (e) { /* ignore */ }
    try { globalThis.iconvLite = require('iconv-lite') } catch (e) { /* ignore */ }
  ...
  test('exports encode/decode and roundtrips', () => {
    ...
    const encodeFn = typeof convjp.encode === 'function' ? convjp.encode : (convjp.default && typeof convjp.default.encode === 'function' ? convjp.default.encode : null)
    const decodeFn = typeof convjp.decode === 'function' ? convjp.decode : (convjp.default && typeof convjp.default.decode === 'function' ? convjp.default.decode : null)
    ...
    try {
      ...
      if (typeof TextDecoder !== 'undefined') localDecoded = new TextDecoder('utf-8').decode(u8)
    ...
    if (decoded === input) {
      ...
      throw new Error('UMD decode did not recover original, but local encoding-japanese conversion does — UMD runtime likely did not resolve the SJIS encoder fallback')
    } else {
